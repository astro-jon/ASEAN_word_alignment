...
class SubWord_Embedding(nn.Module):
    ...
    def Register_subword_word2sub(self, subword_file, lang_size, vocab_dict):
        ...
        special_tokens =["<PAD>", "<MASK>", "<BOS>", "<EOS>","UNK"] #list of SP; MASK not used
        subwords = sorted(list(set(subwords))) + special_tokens #list of SP
        self.subword2id = dict(zip(subwords, range(len(subwords))))
        logger.critical("Number of shared SP: " + str(len(self.subword2id)))
...
class Shared_MT(nn.Module):
    def __init__(self, encoder, decoder, vocab_dict, emb_size, swemb_size, share_vocab, subword_file, dr_rate, pretrained_emb):
        ...
        self.fc2 = torch.nn.Linear(emb_size, emb_size)
    ...
    def Hinton_loss(self, z1: torch.Tensor, z2: torch.Tensor,
    ...
    def InfoNCE_loss(self, z1, z2):
        ...
        loss = F.cross_entropy(logits, labels)  # 默认取平均
        return loss

    def multi_class_n_pair_loss(self, embeddings, alpha = 0.1):
        ...
        # Compute the positive (same-class) and negative (different-class) similarities
        mask = labels.expand(batch_size, batch_size).eq(labels.expand(batch_size, batch_size).t())
        pos_similarities = similarities[mask].view(batch_size, -1)  # 只获得mask为true，即自己与自己的乘积
        neg_similarities = similarities[~mask].view(batch_size, -1)

        # Compute the loss using the N-pair loss objective
        loss = torch.sum(torch.log(1 + torch.exp(-(pos_similarities - neg_similarities + alpha)))) / batch_size

        return loss

    def loss_cal(self, x, x_aug):  # GraphCL
    ...
    def decode(self, t_id, hs, s_lengths, s_id_emb):
        ...
        ht, attn_matix, tgt_ht = self.decoder._decode(t_id_emb, hs, s_lengths, s_id_emb)
        score_sep = self.Output_layer(ht, embedding_weight)
        return score_sep, t_id_emb, ht.view(len(t_id_emb),len(t_id_emb[0]),-1), tgt_ht
